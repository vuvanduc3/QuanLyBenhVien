...
const QuanLyNguoiDungScreen = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/quanlynguoidungbaohiem');
    ...
    const handleAdd = (item) => {
        navigate('/ADDBHYT', { state: { action: 'add', item } });
    };
    const handleEdit = (item) => {
        navigate('/ADDBHYT', { state: { action: 'edit', item } });
    };
